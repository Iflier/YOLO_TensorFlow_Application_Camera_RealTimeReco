...
class YOLO_TF:
    weights_file = r'weights\YOLO_tiny.ckpt'#权重文件路径
    alpha = 0.1
    threshold = 0.15
    iou_threshold = 0.5
    num_class = 20
    ...
    def __init__(self,imshow,disp_console=False):
    ...
    def conv_layer(self,idx,inputs,filters,size,stride):
        ...
        if self.disp_console:
            print("Layer {0}: Type = Conv, Size = {1} * {2},Stride = {3}, Filters = {4}, Input channels = {5}".format(idx,size,size,stride,filters,int(channels)))
        return tf.maximum(self.alpha*conv_biased,conv_biased,name = str(idx)+"_leaky_relu")
    def pooling_layer(self,idx,inputs,size,stride):
        if self.disp_console:
            print("Layer {0}: Type = Pool, Size = {1} * {2}, Stride = {3}".format(idx,size,size,stride))
    ...
    def fc_layer(self,idx,inputs,hiddens,flat = False,linear = False):
        ...
        if self.disp_console:
            print("Layer {0}: Type = Full,Hidden = {1},Input dimension = {2},Flat = {3},Activation = {4}".format(idx,hiddens,int(dim),int(flat),1-int(linear)))
        if linear:
    ...
    def detect_from_crop_sample(self):
        self.w_img = 640
        self.h_img = 420
        f = np.array(open("person_crop.txt","r").readlines(),dtype = np.float32)
        inputs = np.zeros((1,448,448,3),dtype = np.float32)
        for c in range(3):
            for y in range(448):
                for x in range(448):
    ...
    def show_results(self,img,results):
        ...
        for i in range(len(results)):
            ...
            w = int(results[i][3]) // 2
            h = int(results[i][4]) // 2
            print("Target found in:{0},{1},{2},{3}".format(x,y,w,h))
            print("Class: {}".format(results[i][0]))
            if self.disp_console:
                print("class: "+results[i][0]+",[x,y,w,h]=["+str(x)+","+str(y)+","+str(int(results[i][3]))+","+str(int(results[i][4]))+"],Confidence = "+str(results[i][5]))
            if self.imshow:
                cv2.rectangle(img_cp,(x-w,y-h),(x+w,y+h),(0,255,0),2)
                cv2.rectangle(img_cp,(x-w,y-h-20),(x+w,y-h),(255,255,255),-1)
                cv2.putText(img_cp,results[i][0]+":{0:.2f}".format(results[i][5]),(x-w,y-h-5),cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,0,0),1)
        if self.imshow:
            cv2.putText(img_cp,"Press Esc to quit.",(5,30),cv2.FONT_HERSHEY_SIMPLEX,1.0,(0,0,255),1,cv2.LINE_AA)
